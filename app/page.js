...
const InquiryForm = ({ service = false }) => { 
  ...
  return sent ? <div className="rounded-2xl border border-primary/30 bg-primary/10 p-8 text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-primary text-primary-foreground"><Check /></div><h3 className="mt-4 font-display text-2xl font-bold">Message received, thank you.</h3><p className="mt-2 text-sm text-muted-foreground">Our team will get back to you shortly. You can also reach us instantly on WhatsApp.</p></div> : <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2"><input required name="name" placeholder="Your name" className="field" /><input required name="phone" placeholder="Phone number" type="tel" className="field" />{service ? <><input required name="carModel" placeholder="Car model" className="field" /><select required name="subject" className="field" defaultValue=""><option value="" disabled>Service needed</option>{services.map((s) => <option key={s.id}>{s.title}</option>)}</select><input required name="dateNeeded" type="date" className="field" /><textarea name="message" placeholder="Tell us a little more" className="field min-h-28 sm:col-span-2" /></> : <><input type="email" name="email" placeholder="Email address" className="field" /><input name="subject" placeholder="Subject" className="field" /><textarea required name="message" placeholder="How can we help?" className="field min-h-28 sm:col-span-2" /></>}<Button type="submit" disabled={loading} className="sm:col-span-2">{loading ? 'Sending...' : service ? 'Book my service' : 'Send enquiry'} <ArrowRight className="h-4 w-4" /></Button><p className="text-xs text-muted-foreground sm:col-span-2">We respect your time. No spam, just a helpful reply from FAM.</p></form> 
}
